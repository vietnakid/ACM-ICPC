...
int main()
{
    ...
    FORE(tc, 1, TC) {
        ...
        FORE(i, 0, m) {
            ...
            if (index > 1) {
                int y1 = arr[index-1].second - arr[index-2].second;
                int y2 = arr[index].second - arr[index-1].second;
        ...
        if (y1 < 0) {
            arr.insert(arr.begin(), {arr[0].first, arr[0].second});
        }
        m = arr.size();
        // cerr << m << " " << basex << endl;
        if (m % 2 == 0) {
            ii u = arr[arr.size() - 1];
            arr.push_back({u.first, u.second});
        }
        m = arr.size();
        // cerr << m << " new m " << basex << endl;
        ii res = {0, 0};
        FOR(tt, 0, 500) {
            ll midx, midy;
            midx = (basex + minx);
