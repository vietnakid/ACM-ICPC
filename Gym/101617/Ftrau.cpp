...
int main()
{
    ...
    FOR(i, 0, n) {
		for(double angle = 0; angle <= PI*2; angle += 0.005) {
			PT point(0,0);
			point.y = circles[i].second;
			point = RotateCCW(point, angle);
			point.x += circles[i].first.x;
			point.y += circles[i].first.y;
			// cout << point.x << " " << point.y << endl;
			bool ok = true;
			FOR(jj, 0, n) {
				if (dist(point, circles[jj].first) - circles[jj].second >= EPS)
					ok = false; 
			}
			if (ok) {
				res = max(res, dist(point, ZERO));
			}
		}    	
    }
