...
void sovle() {
    ...
    for (int i = 2; i < n-1; i++) {
        ...
        PT C = brr[i];
        PT D = brr[i+1];
        // Neu cat nhau nhung khong cat o giua doan.
        if (!LinesParallel(A, B, C, D) && !SegmentsIntersect(A, B, C, D)) {
            PT intersecPoint = ComputeLineIntersection(A, B, C, D);
...
int main()
{
    ...
    // int curTurn = 0;
    // for (int i = 1; i < whatisit.size()-1; i++) {
    //     bool isLine = false;
    //     if (LinesParallel(whatisit[i-1], whatisit[i], whatisit[i], whatisit[i+1])) {
    //         double d = dist(whatisit[i-1], whatisit[i]) + dist(whatisit[i], whatisit[i+1]);
    //         double dd = dist(whatisit[i-1], whatisit[i+1]);
    //         isLine = abs(dd - d) <= esp; 
    //     }
    //     curTurn += !isLine;
    // }
    // double curRes = 0;
    // for (int i = 1; i < whatisit.size(); i++) {
    //     curRes += dist(whatisit[i-1], whatisit[i]);
    // }
    // cerr << endl;
    // for (auto x: whatisit) {
    //     cerr << x << " ";
    // }
    // cerr << endl;
    // cerr << whatisit.size() << " | turn = " << curTurn << " | res = " <<  curRes << endl;
    // cerr << endl;
    return 0;
}