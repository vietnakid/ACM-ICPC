...
struct Edge {
    int t, u, v;
    ll cost;
    Edge(int t, int u, int v, ll cost) : t(t), u(u), v(v), cost(cost) {}
    ll operator < (const Edge& other) const {
...
int main()
{
    ...
    for (int i = 0; i < p; i++) {
        ...
        SUM = (SUM + mulMod(cost, m)) % nMod;
    }
    for (int i = 0; i < q; i++) {
        ll u, v, cost;
        scanf("%lld %lld %lld", &u, &v, &cost);
        arr.push_back(Edge(2, u, v, cost));
        SUM = (SUM + mulMod(cost, n)) % nMod;
    }
    ...
    for (int i = 0; i < (int)arr.size(); i++) {
        ...
        if (curType == rowType) {
            if (!isSameRoot(rowType, curEdge.u, curEdge.v)) {
                ...
                // cerr << "mulMod " << curEdge.cost << " " << n - numColDrawed << " " << mulMod(curEdge.cost, n - numColDrawed) << endl;
            }
        } else {
            if (!isSameRoot(colType, curEdge.u, curEdge.v)) {
                ...
                // cerr << "mulMod " << curEdge.cost << " " << m - numRowDrawed << " " << mulMod(curEdge.cost, m - numRowDrawed) << endl;
            }
        }
